# Section Summary — Arrays & Strings

- Core concepts: indexing, two-pointers, sliding window, prefix sums, in-place vs extra space.
- Key patterns: two-pointer for sorted arrays, sliding window for subarray problems, hashing for frequency/counting, sorting for reorder/median tasks.
- Complexity tradeoffs: O(n) vs O(n log n) vs O(n^2), in-place constraints.

Review prompts:
- When do you use two-pointers vs hash map vs sorting? Give examples.
- How do you convert an O(n^2) string algorithm to O(n) or O(n log n)?
- Explain sliding window and write steps to detect variable-length windows.
- Describe prefix-sum usage for range-sum queries and subarray counting.
- Practice: implement two-sum, longest substring without repeating characters, product except self, rotate matrix (conceptual steps).
