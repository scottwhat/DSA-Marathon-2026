# Section Summary — Binary Search

- Core concepts: monotonic predicate, search on index/answer space, off-by-one handling.
- Key patterns: standard binary search on sorted arrays, search-in-rotated-array, search-on-matrix, parametric search.
- Implementation tips: prefer while low<=high or while low<high consistently; test boundaries.

Review prompts:
- Explain invariant maintenance for binary search and common off-by-one errors.
- When to binary-search on the answer vs on indexes? Give examples.
- Practice: search in rotated sorted array, find first/last occurrence, find minimum in rotated array.
