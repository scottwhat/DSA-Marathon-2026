# Section Summary — Trees

- Core concepts: tree traversals (pre/in/post, BFS), recursion vs iteration, binary tree vs BST properties.
- Key patterns: DFS for path sums, BFS level-order for distance/level problems, inorder properties for BSTs, tree construction from traversals.
- Complexity: recursion depth and stack usage; convert recursion to iterative with stack when needed.

Review prompts:
- Explain preorder/inorder/postorder and when each is useful.
- How to validate a BST and find LCA efficiently?
- Practice: serialize/deserialize, diameter, path sum variations, invert tree.
