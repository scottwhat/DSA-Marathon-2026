# Section Summary — Sliding Window

- Core concepts: fixed vs dynamic window, maintaining counts/aggregates, expanding and contracting pointers.
- Key patterns: longest/shortest subarray meeting conditions, anagram/subsequence checks with frequency maps.
- Implementation tips: carefully manage when to shrink window and update answer.

Review prompts:
- Distinguish fixed-size vs variable-size sliding window strategies.
- How to maintain window state (counts, sum, max) efficiently?
- Practice: minimum size subarray sum, longest substring without repeating characters, permutation in string.
