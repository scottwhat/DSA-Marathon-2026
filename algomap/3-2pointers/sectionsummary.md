# Section Summary — Two Pointers

- Core concepts: slow/fast pointers, left/right pointers on sorted arrays, in-place partitioning.
- Key patterns: pair-sum in sorted arrays, removing duplicates, reversing, partitioning, window expansions.
- Complexity: usually O(n) time, O(1) extra space.

Review prompts:
- Differentiate slow/fast pointers vs left/right paired pointers.
- Walk through solving 3Sum and how to avoid duplicates.
- Explain how two pointers solve palindrome checks and container-with-most-water.
- Practice: implement trapping rain water (conceptual), merge two sorted arrays in-place.
