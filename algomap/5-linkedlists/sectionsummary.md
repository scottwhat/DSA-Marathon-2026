# Section Summary — Linked Lists

- Core concepts: node pointers, head/tail handling, in-place reversal, slow/fast pointers for cycles.
- Key patterns: detect cycle, reverse list, merge lists, reorder, find middle node.
- Complexity: pointer manipulation mistakes are common — draw diagrams.

Review prompts:
- How to reverse a linked list iteratively and recursively? Trade-offs.
- Explain detecting cycles and finding entry point (Floyd's algorithm).
- Practice: merge k sorted lists, copy list with random pointer (high-level approach).
- Edge cases: empty list, single node, even/odd lengths.
